...
export default createStore({
  ...
  mutations: {
    ...
    ADD_TO_CART(state, pizza) {
      const uniqIdFormStr = pizza.id + '' + pizza.price
      const isPizzaAlreadyAdded = state.cartItems.get(uniqIdFormStr);
      if (isPizzaAlreadyAdded) {
        const modifiedPizza = {
          ...pizza,
          count: isPizzaAlreadyAdded.count + 1,
        };
        state.cartItems.set(uniqIdFormStr, modifiedPizza);
      } else {
        state.cartItems.set(uniqIdFormStr, { ...pizza, count: 1 });
      }
    },
    DEC_TO_CART(state, pizza) {
      ...
      if (isPizzaAlreadyAdded.count <= 1) {
        state.cartItems.delete(uniqIdFormStr);
      }
    },
